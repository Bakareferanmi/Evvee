            ...
            return (
              <Reveal key={cat.slug} delay={i * 0.05}>
                <button
                  ...
                  className="group w-full flex flex-col items-center gap-3 text-center bg-elevated border border-border rounded-lg p-5 hover:border-purple hover:shadow-sm hover:-translate-y-0.5 transition-all duration-200 ease-evvee"
                >
                  <span
                    className={`w-12 h-12 rounded-md flex items-center justify-center text-xl transition-all duration-200 ease-evvee group-hover:scale-110 ${
                      cat.tint === 'purple' ? 'bg-purple-dim text-purple' : 'bg-lime-dim text-lime-deep'
                    } group-hover:bg-purple group-hover:text-white`}
                  >
                    <Icon />
                  </span>
                  <span className="text-sm font-semibold group-hover:text-purple transition-colors">{cat.name}</span>
                </button>
              </Reveal>
            )
          })}
