...
import { FiUser, FiHeart, FiClipboard, FiCheckCircle, FiLogOut, FiArrowRight, FiSun, FiMoon } from 'react-icons/fi'
...
export default function Navbar() {
  const { user, logout, savedVendorIds, bookings, openModal, darkMode, toggleDarkMode } = useApp()
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-[100] py-3.5 bg-bg/80 backdrop-blur-md border-b border-border">
      <div className="container-evvee flex items-center justify-between">
        ...
        <div className="flex items-center gap-2.5">
          <button
            ...
            className="w-10 h-10 rounded-full border border-border flex items-center justify-center text-ink-muted hover:text-ink hover:border-purple transition-colors"
          >
            ...
            <AnimatePresence>
              {dropdownOpen && (
                <motion.div
                  ...
                  className="absolute right-0 mt-2 w-72 bg-elevated rounded-lg shadow-md border border-border p-4 z-[110]"
                >
                  {!user ? (
                    <div>
